...
REPO_NAME = "mace-materials-pipeline"
NOTEBOOK_NAME = "mace_materials_colab.ipynb"
EXPECTED_PROFILE = "E2E"
EXPECTED_MODEL_ID = "mace-foundations/mace-mp-0"
...
# No second 40-hex revision is cited beside the pinned Hub commit (the upstream release asset is a tag).
KNOWN_SHAS: frozenset[str] = frozenset()
BYOD_GATES = ("USE_BYOD",)
EXPECTED_OUTPUTS = (
    "outputs/mace_materials_sample_dataset.xyz",
    "outputs/mace_materials_evaluation_report.json",
...
CODE_MARKERS = (
    ...
    "report=print",
    # Stage 4: generated EMT-labelled sample, validation, composition-stratified split, refusal probes
    "USE_BYOD = False",
    ...
    "lr=LEARNING_RATE",
    "'e0_calibration_eV'",
    # Stage 8: held-out evaluation and comparison
...
MARKDOWN_MARKERS = (
    ...
    "pickled torch module",
    "unpickles it exactly once",
    "code-free JSON config + safetensors pair",
    "analytic gradient",
...
# digest-gated conversion — may call torch.load(weights_only=False). Its body is masked before the fleet
# patterns run, and `_validate_audited_conversion` asserts that the call exists nowhere else, that the
# function runs the static audit and the digest check before it, and that no notebook cell outside the
# carried modules calls it directly.
AUDITED_CONVERSION_FUNCTION = "convert_model"
AUDITED_CONVERSION_GATES = ("audit_model_file(", "SOURCE_MODEL_SHA256", "PICKLE_AUDIT_SHA256")
...
UNSAFE_LOAD = re.compile(r"\btorch\.load\s*\((?![^)]*weights_only\s*=\s*True)|weights_only\s*=\s*False")


def _mask_audited_conversion(path: Path, code_cells: list[tuple[int, str, ast.Module]], embedded: list[int]) -> dict[int, str]:
    """Return comment-stripped cell sources with the audited conversion function's body blanked out, after
    checking that it is defined exactly once in a carried module cell and gates the unsafe load."""
    ...
    for index, source, tree in code_cells:
        ...
        if index in embedded:
            for node in tree.body:
                if isinstance(node, ast.FunctionDef) and node.name == AUDITED_CONVERSION_FUNCTION:
                    definitions += 1
                    lines = source.splitlines(keepends=True)
                    body = "".join(lines[node.lineno - 1 : node.end_lineno])
                    _check(UNSAFE_LOAD.search(body) is not None, f"{path.name}: {AUDITED_CONVERSION_FUNCTION} must be where the single unsafe load lives")
                    load_at = max(match.start() for match in UNSAFE_LOAD.finditer(body))  # the call, not its docstring mention
                    for gate in AUDITED_CONVERSION_GATES:
                        _check(0 <= body.find(gate) < load_at, f"{path.name}: {AUDITED_CONVERSION_FUNCTION} must apply {gate} before the unsafe load")
                    text = _strip_comments("".join(lines[: node.lineno - 1]) + "".join(lines[node.end_lineno :]))
        stripped[index] = text
    _check(definitions == 1, f"{path.name}: {AUDITED_CONVERSION_FUNCTION} must be defined exactly once, in a carried module cell")
    return stripped
